...
fn star_network_create(num: usize) -> Network {
    ...
    let mut network: HashMap<_, _> = (1..num)
        .map(|_| {
            ...
            let node = CrdsGossip::default();
            {
                ...
                node_crds
                    .insert(new.clone(), timestamp(), GossipRoute::LocalMessage, None)
                    .unwrap();
                node_crds
                    .insert(entry.clone(), timestamp(), GossipRoute::LocalMessage, None)
                    .unwrap();
            }
    ...
    node.crds
        .write()
        .unwrap()
        .insert(entry, timestamp(), GossipRoute::LocalMessage, None)
        .unwrap();
    let node = Node::new(node_keypair, contact_info, Arc::new(node));
...
fn ring_network_create(num: usize) -> Network {
    ...
    for k in 0..keys.len() {
        ...
        end_crds
            .insert(start_info, timestamp(), GossipRoute::LocalMessage, None)
            .unwrap();
    }
    Network::new(network)
}
    ...
    for t in start..end {
        ...
        let requests: Vec<_> = network_values
            ...
            .map(|node| {
                ...
                (
                    node_pubkey,
                    node.gossip.new_push_messages(&node_pubkey, now, &stakes, None).0,
                )
            })
            .collect();
