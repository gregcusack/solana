...
use {
    ...
    agave_votor::{
        ...
        voting_service::{
            VotingService as BLSVotingService, VotingServiceOverride, VotorSendMode,
            VotorTransportMode,
        },
...
pub struct Tvu {
    ...
    drop_bank_service: DropBankService,
    duplicate_shred_listener: DuplicateShredListener,
    bls_transport_service: Option<BlsTransportService>,
    votor: Votor,
    commitment_service: AggregateCommitmentService,
}

enum BlsTransportService {
    Stream {
        streamer: JoinHandle<()>,
        sigverifier: JoinHandle<()>,
    },
    Datagram {
        sigverifier: JoinHandle<()>,
    },
...
impl BlsTransportService {
    fn join(self) -> thread::Result<()> {
        match self {
            ...
            Self::Datagram { sigverifier } => sigverifier.join(),
        }
    }
}
...
pub struct AlpenglowInitializationState {
    ...
    // For BLS voting service stream mode
    ...
    pub votor_egress: tokio::sync::mpsc::Sender<Datagram>,
...
impl Tvu {
    ...
    pub fn new(
        ...
        let bls_transport_service = match tvu_config.votor_transport_mode {
            VotorTransportMode::QuicStream => bls_socket.map(|bls_socket| {
                ...
                let bls_sigverifier_t = bls_sigverifier::spawn_service(
                    ...
                    SigVerifierContext {
                        ...
                        banlist: bls_sigverifier::BlsBanlist::Stream(banlist),
                        sharable_banks,
                    ...
                    SigVerifierChannels {
                        packet_receiver: bls_sigverifier::BlsPacketReceiver::Stream(
                            bls_packet_receiver,
                        ),
                ...
                BlsTransportService::Stream {
                    streamer: bls_streamer_t,
                    sigverifier: bls_sigverifier_t,
                }
            }),
            VotorTransportMode::QuicDatagram => {
                ...
                let bls_sigverifier_t = bls_sigverifier::spawn_service(
                    ...
                    SigVerifierContext {
                        migration_status: migration_status.clone(),
                        banlist: bls_sigverifier::BlsBanlist::Datagram(votor_banlist),
                        sharable_banks,
                        cluster_info: cluster_info.clone(),
                        leader_schedule: leader_schedule_cache.clone(),
                        num_threads: tvu_config.bls_sigverify_threads.get(),
                    ...
                    SigVerifierChannels {
                        packet_receiver: bls_sigverifier::BlsPacketReceiver::Datagram(
                            votor_ingress,
                        ),
                        channel_to_repair: verified_voter_slots_sender,
                        channel_to_reward: reward_votes_sender,
                ...
                Some(BlsTransportService::Datagram {
                    sigverifier: bls_sigverifier_t,
                })
            }
        };
        ...
        let bls_voting_service = BLSVotingService::new(
            ...
            match tvu_config.votor_transport_mode {
                VotorTransportMode::QuicStream => VotorSendMode::Stream {
                    connection_cache: bls_connection_cache,
                },
                VotorTransportMode::QuicDatagram => VotorSendMode::Datagram {
                    egress: votor_egress,
    ...
    pub fn join(self) -> thread::Result<()> {
        ...
        if let Some(bls_transport_service) = self.bls_transport_service {
            bls_transport_service.join()?;
        }
        self.votor.join()?;
    ...
    fn test_tvu_exit() {
        ...
        let tvu = Tvu::new(
            ...
            AlpenglowInitializationState {
                ...
                key_notifiers,
                bls_connection_cache: Arc::new(ConnectionCache::new_quic_for_tests(
