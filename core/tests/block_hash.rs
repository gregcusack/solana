...
fn test_bank_hash_equivalence(snapshot_version: SnapshotVersion, cluster_type: ClusterType) {
    solana_logger::setup();
    info!("hello world");
    ...
    for slot in 1..=last_slot {
        ...
        let bank = bank_forks.write().unwrap().insert(bank);
        let bank_no_scheduler = bank.clone_without_scheduler();
        let key1 = Keypair::new().pubkey();
        let tx = system_transaction::transfer(mint_keypair, &key1, 1, bank_no_scheduler.last_blockhash());
        assert_eq!(bank_no_scheduler.process_transaction(&tx), Ok(()));

        let key2 = Keypair::new().pubkey();
        let tx = system_transaction::transfer(mint_keypair, &key2, 0, bank_no_scheduler.last_blockhash());
        assert_eq!(bank_no_scheduler.process_transaction(&tx), Ok(()));

        bank_no_scheduler.freeze();
